...
export const FauxCurrentUser = React.createContext({
  user_id: "66ba6bfd81833a6900354b86",
});
  ...
  useEffect(() => {
    const fetchUnreadReports = async () => {
      ...
      const unreadUnclaimedReports = reports.unclaimed.filter(
        (r: Report) => !r?.read_by?.includes(user_id)
      ); // Filter unclaimed reports where user_id is not in read_by
