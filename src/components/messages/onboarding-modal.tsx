...
const OnboardingModal = ({
  ...
  // update the bg theme
  useEffect(() => {
    if (step < 4) updateThemeColor("#122c57");
    if (step >= 3 && isVisible === false) updateThemeColor("#1c4587");
    if (step === 0 && isNoClicked) updateThemeColor("#1c4587");
  }, [isVisible]);
  ...
  return (
    <>
      ...
      <div
        id="modal"
        className={`ease-in-out w-4/5 z-30 fixed ${
          isVisible && (step === 1 || step === 2) ? "top-[50%]" : "top-[40%]"
        } ${
          isVisible && step === 3 ? "top-[200px]" : "top-[40%]"
        } left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-[28px] transition-all duration-700 ${
      ...
      <div
        id="overlay"
        className={`fixed w-screen h-screen z-10 transition-opacity duration-700 ${
          isVisible ? "bg-black opacity-35" : "opacity-0"
        } ${step < 3 && !isNoClicked && "bg-black inset-0 opacity-35"}`}
      />
      {/* Report Spotlight */}
      <div
        id="document-icon-spotlight"
        className={`fixed w-14 h-14 z-20 top-2 right-3 transition-shadow invisible shadow-[0_0_0_9999px_rgba(0,0,0,0.35)] ${shadowClass}`}
      />
    </>
  );
};
