...
export default function ConnectedReportsField() {
    const [connectedReport, setConnectedReport] = useState([]);
    ...
    const handleReportChange = (e, report) => {
        if (e.target.checked) {
    ...
    return (<>
      <fieldset className="border p-1 rounded-md border-black">
        ...
        <div className="relative flex flex-row">
            ...
            <input type="text" placeholder="Select any connected reports" className="block w-full appearance-none bg-white placeholder-wrap pb-4" value={connectedReport} readOnly/>
            <Image src={dropdownIcon} alt="Icon"/>
          </div>
        </button>
      </fieldset>

      {showReport ? (<div className="w-full flex flex-col bg-gray-100 px-8 py-4 gap-3 rounded-lg shadow-md">
          <small>Select all that apply</small>

          {reports.map((report, index) => (<div key={index}>
              <input type="checkbox" className="w-4 h-4" value={report} onChange={(e) => handleReportChange(e, report)}/>
