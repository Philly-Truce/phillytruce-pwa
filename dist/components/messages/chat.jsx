...
export default function Home({ conversationProxy, onboardingStep, }) {
    ...
    return (<div id="message-input-container" className="flex flex-col gap-3 justify-between h-full">
      <div id="message-bubbles-container" className="flex-grow overflow-y-auto flex flex-col gap-[14px] pt-4 scrollbar scrollbar-thumb-[#879cbd] scrollbar-w-16">
        {messages.map((message, index) => {
            var _a;
            const friendlyName = generateFriendlyChatName(message.author);
            const isLastMessage = index === messages.length - 1;
            const isLastChatbotMessage = friendlyName === "Chatbot" &&
                messages
                    .slice(index + 1)
                    .every((m) => generateFriendlyChatName(m.author) !== "Chatbot");
            return (<div key={index} className="flex flex-col">
              <div id="message-bubble" ref={isLastMessage ? lastMessageRef : null} className={`p-4 rounded-[15px] max-w-[255px] ${friendlyName === "Chatbot" || friendlyName === "You"
                    ? "bg-[#b3bfd3] self-end mr-4"
                    : "bg-white ml-4"}`}>
                <div id="message-content" className="text-[13px] leading-[166%] tracking-[0.4px]">
                  {message.body}
                </div>
                <div id="message-date-created" className="text-[11px] leading-[166%] tracking-[0.4px] text-right">
                  {(_a = message.dateCreated) === null || _a === void 0 ? void 0 : _a.toLocaleString("en-US", {
                    hour: "numeric",
                    minute: "2-digit",
                    hour12: true,
                })}
              ...
              {isLastChatbotMessage && (<div id="divider-wrapper" className="relative z-30">
                  <div id="divider-spotlight" className={`absolute inset-0 ${onboardingStep < 3 && onboardingStep > 0
                        ? "bg-white bg-opacity-80"
                        : ""} h-16 top-2 ${onboardingStep === 1
                        ? "top-2"
                        : onboardingStep === 2
                            ? "!top-14"
                            : onboardingStep > 2
                                ? ""
                                : ""}`}/>
                  <div id="chatbot-divider" className="flex flex-col mb-4 mt-[30px] text-[#1C4587] text-center text-[11px] font-bold leading-[166%] tracking-[0.4px] mx-4 relative">
                    <div id="end-chatbot" className="pb-[14px] border-b-4 border-dotted border-[#1C4587]">
