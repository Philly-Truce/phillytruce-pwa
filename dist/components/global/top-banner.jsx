...
const ReportIcon = () => (<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export default function TopBanner() {
    ...
    if (["/login", "/login-otp", "/more"].includes(pathname))
        return null;
    const getPageTitle = () => {
    ...
    const getLeftChild = () => {
        ...
        return (<div id="circle-background" className="rounded-full w-[34px] h-[34px] bg-[#727272] relative flex gap-1">
        <div id="initials" className="absolute text-white text-center text-xs font-bold leading-[100%] tracking-[0.5px] top-1/2 left-1/2 -translate-x-2/4 -translate-y-2/4">
          AV
        </div>
      </div>);
    };
    const getRightChild = () => {
        if (pathname.startsWith("/messages/")) {
            return (<div id="report-icon-wrapper" className="m-3">
          <ReportIcon />
        </div>);
        }
        if (pathname.startsWith("/reports/") || pathname.startsWith("/reports-view")) {
            return (<div id="report-icon-wrapper" className="flex flex-row justify-center items-center">
    ...
    return (<div id="top-banner" className="px-[18px] py-2 bg-primary text-center items-center fixed w-full flex justify-between min-h-16 gap-1 max-w-[640px]">
      {getLeftChild()}
      <h1 className={`text-white text-[22px] font-normal leading-7 flex-grow ${pathname.startsWith("/messages/") || pathname.startsWith("/profile")
            ? "text-left"
            : ""}`}>
        {getPageTitle()}
      </h1>
